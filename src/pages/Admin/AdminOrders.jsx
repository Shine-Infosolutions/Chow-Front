...
const AdminOrders = () => {
  const { getAllOrders, updateOrderStatus, updateItemStock } = useApi();
    ...
    try {
      ...
      if (newStatus === 'delivered') {
        ...
        for (const item of orderItems) {
          try {
            const stockResponse = await updateItemStock(item.itemId || item._id, -item.quantity);
            console.log(`Stock updated for item ${item.name}:`, stockResponse);
          } catch (error) {
            console.error(`Error updating stock for item ${item.name}:`, error);
            alert(`Warning: Could not update stock for ${item.name}`);
          }
        }
      }
            ...
            <table className="min-w-full">
              <thead className="bg-[#d80a4e] text-white">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold uppercase">Customer</th>
                ...
                {orders.map((order, index) => (
                  <tr key={order._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {order.customerId?.name || order.customerName || 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">
